...
function Clickable(x,y) {
	...
	this.textSize = 40;		//Size for the text shown
	...
	this.draw = function () {
		...
		rect(this.x-drawingContext.getTransform().e, this.y-drawingContext.getTransform().f, this.width, this.height, this.cornerRadius);
		fill(this.textColor);
		noStroke();
		if(this.image){
			this.drawImage();
